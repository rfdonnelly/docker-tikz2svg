...
A Docker image that converts a TikZ picture to a JPEG, PDF, PNG, or SVG.

== Build

 docker build -t tikz2image

== Run

=== JPEG

 docker run --rm -i tikz2image jpeg <input.tex >output.jpeg

=== PDF

 docker run --rm -i tikz2image pdf <input.tex >output.pdf

=== PNG

 docker run --rm -i tikz2image png <input.tex >output.png

=== SVG

 docker run --rm -i tikz2image svg <input.tex >output.svg
